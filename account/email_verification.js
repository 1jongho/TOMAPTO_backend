...
router.post('/verify-reset-code', async (req, res) => {
  try {
    ...
    console.log(
      `비밀번호 재설정 인증 시도: 아이디 = ${user_id}, 이메일 = ${email}, 코드 = ${code}`
    );
      ...
      (err, userResults) => {
        ...
        db.query(
          ...
          (err, codeResults) => {
            ...
            console.log(
              `비밀번호 재설정 인증 조회 결과: ${codeResults.length}개 항목 찾음`
            );
        ...
        db.query(
          ...
          async (err, userResults) => {
            ...
            try {
              ...
              // 기존 비밀번호와 동일한지 확인
              if (userResults[0].user_password === hashedNewPassword) {
                return res.status(400).json({
              ...
              // 비밀번호 업데이트
              db.query(
                'UPDATE users SET user_password = ?, updated_at = NOW() WHERE user_id = ? AND user_email = ?',
                [hashedNewPassword, user_id, email],
                (err, updateResult) => {
                  if (err) {
                  ...
                  if (updateResult.affectedRows === 0) {
                    return res.status(400).json({
                      message:
                        '비밀번호 변경에 실패했습니다. 사용자 정보를 확인해주세요.',
                    });
                  }

                  // 인증 코드 삭제 (보안상 사용된 코드는 삭제)
                  db.query(
              ...
              return res
                .status(500)
                .json({ message: '비밀번호 처리 중 오류가 발생했습니다.' });
            }
          }
        );
      }
