...
router.post('/signup', async (req, res) => {
  try {
    const { user_name, user_id, user_nickname, user_password, user_email } = req.body;
    
    // Validate required fields
    if (!user_name || !user_id || !user_nickname || !user_password || !user_email) {
      return res.status(400).json({ message: '모든 필드를 입력해주세요.' });
    }
    
    // Check for duplicate ID
    const [idResults] = await db.promise().query(
    ...
    const [nicknameResults] = await db.promise().query(
      'SELECT COUNT(*) as count FROM users WHERE user_nickname = ?', 
      [user_nickname]
    );
    ...
    await db.promise().query(
      ...
      [user_name, user_id, user_nickname, user_password, user_email]
    );
